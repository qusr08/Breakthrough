...
public class GameSettings : ScriptableObject {
	...
	public string GameSettingsCode {
		get {
			...
			gameSettingsLong =
				(long) ALLOWED_MINOS_RANGE * (GAME_LEVEL_RANGE * (BOOM_BLOCK_CHANCE_RANGE * (WALL_STRENGTH_MULTIPLIER_RANGE *
				(HAZARD_SPEED_MULTIPLIER_RANGE * (MINO_SPEED_MULTIPLIER_RANGE * (BOARD_HEIGHT_RANGE * (boardWidthValue) +
				boardHeightValue) + minoSpeedMultiplierValue) + hazardSpeedMultiplierValue) + wallStrengthMultiplierValue) +
		...
		private set {
			...
			wallStrengthMultiplierValue = (int) (settingsLong % WALL_STRENGTH_MULTIPLIER_RANGE);
			settingsLong /= WALL_STRENGTH_MULTIPLIER_RANGE;
	...
	private void OnValidate ( ) {
		GameSettingsCode = GameSettingsCode; // Remove this at some point its stupid >:(
	}

	//	https://www.stum.de/2008/10/20/base36-encoderdecoder-in-c/
	/// <summary>
	///		Encode the current game settings to a string of characters
	/// </summary>
	/// <param name="input"></param>
	/// <returns>A game settings code in string form</returns>
	...
	/// <summary>
	///		Decode a game settings code of characters into the actual values of the game settings
	/// </summary>
	/// <param name="input"></param>
	/// <returns>An integer that can be further decoded into the game settings values</returns>
	private long Decode (string input) {
		...
		foreach (char c in reversed) {
			result += CHAR_LIST.IndexOf(c) * (long) Mathf.Pow(36, pos);
			pos++;
		}
