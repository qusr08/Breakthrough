...
public class BlockGroup : MonoBehaviour {
    ...
    public List<Block> Blocks => transform.GetComponentsInChildren<Block>( ).ToList( );
    #endregion
    ...
    protected virtual void UpdateTransform ( ) {
        ...
        isDoneTweening = (Utils.CompareVectors(transform.position, toPosition) && Utils.CompareVectors(transform.eulerAngles, toRotation));
    }

    protected bool TryMove (Vector2Int deltaPosition) {
        for (int i = Count - 1; i >= 0; i--) {
            Debug.Log("Try Move Block Group - " + i);
            if (!IsValidBlockPosition(GetBlock(i), GetBlock(i).Position + deltaPosition)) {
                return false;
            }
        }
